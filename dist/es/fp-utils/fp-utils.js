...
var Map = getNative(root, 'Map');/* Built-in method references that are verified to be native. */
...
Stack.prototype.set = stackSet;var defineProperty = (function() {
  try {
 ...
 */
var baseFor = createBaseFor();/** Detect free variable `exports`. */
...
var getPrototype = overArg(Object.getPrototypeOf, Object);/** Used for built-in method references. */
...
var WeakMap$1 = getNative(root, 'WeakMap');/** `Object#toString` result references. */
 ...
 * // => [4, 1]
 */
var over = createOver(arrayMap);/** `Object#toString` result references. */
...
const stubString = ()=>'';
const stubTrue = ()=>true;
const stubFalse$1 = ()=>false;
const noop = ()=>{};
const range=(end=10,start=0,step=1,mapper=identity$1)=>{
  const result=[];
...
const invokeObjectWithArgs = (obj)=>(...args) => mapValues(fn=>isFunction$1(fn) ? fn(...args) : fn)(obj);
...
const over$1 = x=>isArray$1(x)?overArray(x):overObj(x);
const converge = over$1;//backwards compat;

// casting
const constant$1 = x=>_=>x;
const ensureArray = (val=[])=>isArray$1(val) ? val : [val];
const ensureString = (val)=>isString(val) ? val : `${val}`;
const ensureFunction = (arg)=>typeof arg==='function'?arg:constant$1(arg);
...
const mapToArray$1=fn=>transToArray((a,v,k)=>a[a.length]=fn(v,k)); // _ equiv map
const ma=mapToArray$1; // backward compatability
const mapToObject=fn=>transToObject((a,v,k)=>a[k]=fn(v,k)); // _ equiv mapValues
const mo=mapToObject; // backward compatability
const mapToSame=makeCollectionFn(mapToArray$1,mapToObject);
...
const partitionObject = (...preds)=>over([...(preds.map(p=>fo(p))),fo(none(...preds))]);
...
const pget = cond( // polymorphic get
  ...
  [stubTrue,identity$1], // handles the function case
);
const pick=cond(
...
const tdValue = transducer=>value=>transduce(undefined, identity$1, transducer, [value]);
...
const tdIfElse=(pred,tdT,tdF=identity$1)=>nextReducer=>ifElse(pred,tdT(nextReducer),tdF(nextReducer));
const tdCond=acceptArrayOrArgs(predTransducerPairs=>{
  ...
  if(predTransducerPairs.length===1) // single function passed, act like filterWithAcc
    return ifElse(predTransducerPairs[0],identity$1,noop);
...
// lodash equivalents
const memoize$1 = (fn, by = identity$1) => {
...
const diffBy = (by=x=>x.id, args = []) => by ? diffObjs(...args.map(keyBy(by))) : diffObjs(args);export{acceptArrayOrArgs,and,appendArrayReducer,appendObjectReducer,compose,cond,condNoExec,constant$1 as constant,converge,curry,diffBy,diffObjs,dpipe,ensureArray,ensureFunction,ensureProp,ensurePropIsArray,ensurePropIsObject,ensurePropWith,ensureString,fa,filterMapToArray,filterMapToObject,filterMapToSame,filterToArray,filterToObject,filterToSame,first,fma,fmo,fmx,fo,frozenEmptyArray,frozenEmptyObject,fx,groupBy,groupByKeys,groupByValues,has,identity$1 as identity,ifElse,ifElseUnary,immutableFilterObjectToObject,immutableTransArrayToArray,immutableTransObjectToObject,invokeArgsOnObj,invokeObjectWithArgs,is,isArray$1 as isArray,isDeepEqual,isError,isFalsy,isFunction$1 as isFunction,isInteger,isNumber,isObjectLike$1 as isObjectLike,isProductionEnv,isPromise,isString,isTruthy,isUndefOrNull,keyBy,last,len,len0,len1,ma,mapToArray$1 as mapToArray,mapToObject,mapToSame,memoize$1 as memoize,mergeToBlank,mo,mx,none,noop,not,oa,objStringifierFactory,objToUrlParams,omitToArray,omitToObject,omitToSame,oo,or,over$1 as over,overArray,overObj,ox,partition,pget,pick,pipe,plog,ra,range,reduce,renameProps,reverse,ro,rx,slice,sort,stubArray$1 as stubArray,stubFalse$1 as stubFalse,stubNull,stubObject,stubString,stubTrue,tdCond,tdDPipeToArray,tdDPipeToObject,tdDfObjectLikeValuesWith,tdFilter,tdFilterWithAcc,tdIdentity,tdIfElse,tdIfValueObjectLike,tdKeyBy,tdLog,tdMap,tdMapWithAcc,tdNormalizePromises,tdOmit,tdOmitWithAcc,tdPipeToArray,tdPipeToObject,tdReduce,tdReduceListValue,tdTap,tdToArray,tdToObject,tdToSame,tdValue,transToArray,transToObject,transToSame,transduce,transduceBF,transduceDF,uniqueId,xor};