(function(g,f){typeof exports==='object'&&typeof module!=='undefined'?f(exports,require('react')):typeof define==='function'&&define.amd?define(['exports','react'],f):(g=typeof globalThis!=='undefined'?globalThis:g||self,f(g.ReduxGraphql={},g.react));}(this,(function(exports, react){'use strict';// curry/compose/pipe, for later fns
let compose;
const identity=x=>x;
...
var Set$1 = getNative(root, 'Set');/* Built-in method references that are verified to be native. */
...
function isSymbol(value) {
  return typeof value == 'symbol' ||
    (isObjectLike$1(value) && baseGetTag(value) == symbolTag$3);
...
// Expose `MapCache`.
memoize$1.Cache = MapCache;/** Used as the maximum memoize cache size. */
...
function baseMatchesProperty(path, srcValue) {
  ...
  return function(object) {
    ...
    return (objValue === undefined && objValue === srcValue)
      ? hasIn(object, path)
      : baseIsEqual(srcValue, objValue, COMPARE_PARTIAL_FLAG$4 | COMPARE_UNORDERED_FLAG$2);
  };
...
function baseIsMatch(object, source, matchData, customizer) {
  ...
  while (++index < length) {
    ...
    if (noCustomizer && data[2]) {
      ...
      if (!(result === undefined
            ? baseIsEqual(srcValue, objValue, COMPARE_PARTIAL_FLAG$5 | COMPARE_UNORDERED_FLAG$3, customizer, stack)
            : result
          )) {
        return false;
      }
...
const filterIteratees = cond(
  ...
  [isArray,matchesProperty],
  [isObjectLike,matches],
  [stubTrue,k=>v=>k in v],
);
const getMemoizedObjectQuerier=(
  schema,
  queryMatchers={
...
const triggerStorePermutationUpdates = (store,schema,query,act)=>{
