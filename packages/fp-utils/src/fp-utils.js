...
export const isFinite =
  Number.isFinite || ((v) => typeof v === "number" && isFinite(v));
export const isInteger =
  Number.isInteger || ((v) => isFinite(v) && v % 1 === 0);
...
// debugging
export const plog = (msg = "") => (pipeVal) =>
  console.log(msg, pipeVal) || pipeVal;
...
export const overObj = (fnsObj = {}) => (...args) =>
  mapToObject((f) => f(...args))(fnsObj);
export const overArray = (fnsArray = []) => (...args) =>
  mapToArray((f) => f(...args))(fnsArray);
...
export const ensureFunction = (arg) =>
  typeof arg === "function" ? arg : constant(arg);
export const ensureProp = (obj, key, val) => {
  Object.prototype.hasOwnProperty.call(obj, key) ? obj[key] : (obj[key] = val);
  return obj;
};
export const ensurePropWith = (fn) => (obj, key, val) =>
  ensureProp(obj, key, fn(obj, key, val));
export const ensurePropIsArray = ensurePropWith(stubArray);
export const ensurePropIsObject = ensurePropWith(stubObject);

// logic
export const not = (fn) => (...args) => !fn(...args);
export const ifElse = (pred, T, F = identity) => (...args) =>
  (pred(...args) ? T : F)(...args);
...
export const xor = (...preds) => (...args) => {
  let p,
    trues = 0;
  for (p of preds) p(...args) === true && ++trues;
  return trues === 1;
};
export const _is = (x) => (y) => x === y;
export const hasKey = (k = "") => (coll = {}) => k in coll;
export const matchesProperty = ([k, v] = []) => (o = {}) => o[k] === v;
export const matches = (coll = {}) =>
  and(...mapToArray((v, k) => matchesProperty([k, v]))(coll));
export const none = (...preds) => not(or(...preds));
export const condNoExec = (...arrs) => (...x) => {
  for (const [pred, val] of arrs) if (pred(...x)) return val;
};
export const cond = (...arrs) => (...x) => {
  for (const [pred, fn] of arrs) if (pred(...x)) return fn(...x);
};
...
export const immutableFilterObjectToObject = (pred = stubTrue) => (
  coll = {}
) => {
  let k,
    acc = {},
    changed = false;
  for (k in coll)
    pred(coll[k], k, coll) ? (acc[k] = coll[k]) : (changed = true);
  return changed === true ? acc : coll;
};

export const transToObject = (...fn) =>
  ifElse(isArray, transArrayToObject(...fn), transObjectToObject(...fn));
export const transToArray = (...fn) =>
  ifElse(isArray, transArrayToArray(...fn), transObjectToArray(...fn));
export const transToSame = (...fn) =>
  ifElse(isArray, transArrayToArray(...fn), transObjectToObject(...fn));
export const filterToArray = (pred) =>
  transToArray((a, v, k) => pred(v, k) && (a[a.length] = v)); // _ equiv filter
export const filterToObject = (pred) =>
  transToObject((a, v, k) => pred(v, k) && (a[k] = v)); // _ equiv pickBy
export const filterToSame = (...fn) =>
  ifElse(isArray, filterToArray(...fn), filterToObject(...fn));
export const omitToArray = (pred) =>
  transToArray((a, v, k) => !pred(v, k) && (a[a.length] = v)); // _ equiv withoutBy
export const omitToObject = (pred) =>
  transToObject((a, v, k) => !pred(v, k) && (a[k] = v)); // _ equiv pickBy
export const omitToSame = (...fn) =>
  ifElse(isArray, omitToArray(...fn), omitToObject(...fn));
export const mapToArray = (fn) =>
  transToArray((a, v, k) => (a[a.length] = fn(v, k))); // _ equiv map
export const mapToObject = (fn) =>
  transToObject((a, v, k) => (a[k] = fn(v, k))); // _ equiv mapValues
export const mapToSame = (...fn) =>
  ifElse(isArray, mapToArray(...fn), mapToObject(...fn));
export const filterMapToArray = (pred, mapper) =>
  transToArray((a, v, k) => pred(v, k) && (a[a.length] = mapper(v, k)));
export const filterMapToObject = (pred, mapper) =>
  transToObject((o, v, k) => pred(v, k) && (o[k] = mapper(v, k)));
export const filterMapToSame = (...fn) =>
  ifElse(isArray, filterMapToArray(...fn), filterMapToObject(...fn));

export const first = (c) => (isArray(c) ? c : Object.values(c))[0];
export const last = (c) => (isArray(c) ?? (c = Object.values(c)))[c.length - 1];
const partitionObject = (...preds) =>
  overArray([
    ...preds.map((p) => filterToObject(p)),
    filterToObject(none(...preds)),
  ]);
const partitionArray = (...preds) =>
  overArray([...preds.map(filterToArray), filterToArray(none(...preds))]);
export const partition = (...preds) =>
  ifElse(isArray, partitionArray(...preds), partitionObject(...preds));

// collection predicates
export const isLength = (num) => (x) =>
  num ===
  ((isArray(x) && x.length) ??
    (isObjectLike(x) && Object.keys(x).length) ??
    (isFunction(x) && x.length) ??
    0);
export const has = (key) => (obj) => key in obj;

// indexers
export const keyBy = ifElse(
  isString,
  (id = "id") => keyBy((x) => x[id]),
  (fn = (x) => x.id) =>
    transToObject((o, v, k) => {
...
export const indexBy = (...keyGetters) => (coll) => {
  ...
  const getter =
    typeof keyGetters[0] === "string" ? (v) => v[keyGetters[0]] : keyGetters[0];
  return mapToObject(indexBy(...keyGetters.slice(1)))(
    transToObject((o, v, k) => {
      const kk = getter(v, k);
      (o[kk] ?? (o[kk] = {}))[k] = v;
    })(coll)
...
const pushToArray = (a = [], v) => {
  a[a.length] = v;
  return a;
};
const pushToArrayProp = (acc = {}, v, k) => {
  acc[k] = pushToArray(acc[k], v);
  return acc;
};
export const groupBy = (fn) =>
  transToObject((o, v, k) => pushToArrayProp(o, v, fn(v, k)));
export const groupByKeys = transToObject((o, v, k) => {
  for (k in v) pushToArrayProp(o, v, k);
});
export const groupByValues = transToObject((o, v) => {
  let k, vv;
  for (k in v)
    for (vv of ensureArray(v[k])) pushToArrayProp(o, v, ensureString(vv));
});
...
export const pget = cond(
  ...
  [
    ...
    (str) => {
      ...
      return (targ) =>
        str.reduce(
          (t, s) => (isArray(t) && !isInteger(+s) ? t.map((o) => o[s]) : t[s]),
          targ
        );
    },
  ],
...
export const pick = cond(
  [
    isArray,
    (keys) => (obj) => transArrayToObject((o, k) => (o[k] = obj[k]))(keys),
  ],
  [isString, (key) => (obj) => ({ [key]: obj[key] })],
  [isFunction, filterToSame],
  [
    stubTrue,
    (keys) => () => new Error("unsupported type for pick: " + typeof keys),
  ]
);

// content
export const uniqueId = (start = 0, fn = identity) => (...args) =>
  fn(start++, ...args);
...
export const tdToArray = (transducer) => (collection) =>
  transduce([], appendArrayReducer, transducer, collection);
export const tdToObject = (transducer) => (collection) =>
  transduce({}, appendObjectReducer, transducer, collection);
export const tdToSame = (transducer) => (collection) =>
  (Array.isArray(collection) ? tdToArray : tdToObject)(transducer)(collection);
export const tdMap = (mapper) => (nextReducer) => (a, v, ...kc) =>
  nextReducer(a, mapper(v, ...kc), ...kc);
export const tdMapKey = (mapper) => (nextReducer) => (a, v, k, ...c) =>
  nextReducer(a, v, mapper(v, k, ...c), ...c);
export const tdMapWithAcc = (mapper) => (nextReducer) => (a, v, ...kc) =>
  nextReducer(a, mapper(a, v, ...kc), ...kc);
export const tdMapKeyWithAcc = (mapper) => (nextReducer) => (a, v, k, ...c) =>
  nextReducer(a, v, mapper(a, v, k, ...c), ...c);
export const tdAssign = (f) => (nextReducer) => (a, v, ...kc) =>
  nextReducer({ ...a, ...f(a, v, ...kc) }, v, ...kc);
export const tdSet = (key, f) => (nextReducer) => (a, v, ...kc) => {
  const next = f(a, v, ...kc);
  return a[key] === next
    ? nextReducer(a, v)
    : nextReducer({ ...a, [key]: next }, v, ...kc);
};
...
export const tdLog = (msg = "log", pred = stubTrue) =>
  tdTap((...args) => pred(...args) && console.log(msg, ...args));
export const tdFilter = (pred = stubTrue) => (nextReducer) => (a, ...args) =>
  pred(...args) ? nextReducer(a, ...args) : a;
export const tdFilterWithAcc = (pred = stubTrue) => (nextReducer) => (
  ...args
...
export const tdDPipeToArray = (coll, ...fns) =>
  tdToArray(compose(...fns))(coll);
export const tdDPipeToObject = (coll, ...fns) =>
  tdToObject(compose(...fns))(coll);
export const tdReduceListValue = (nextReducer) => (acc, v, k, ...args) => {
  if (!isObjectLike(v)) return nextReducer(acc, v, k, ...args);
  if (isArray(v))
    for (let kk = -1, l = v.length; ++kk < l; )
      acc = nextReducer(acc, v[kk], kk, v);
  else for (const kk in v) acc = nextReducer(acc, v[kk], kk, v);
  return acc;
};
export const reduce = (fn) => (coll, acc) => tdReduceListValue(fn)(acc, coll);
export const tdIfElse = (pred, tdT, tdF = identity) => (nextReducer) =>
  ifElse(pred, tdT(nextReducer), tdF(nextReducer));
export const transduceDF = ({
  preVisit = tdIdentity,
  visit = (nextReducer) => (a, v, k, c, df) =>
    nextReducer(a, isObjectLike(v) ? df({}, v) : v, k, c),
  postVisit = tdIdentity,
  edgeCombiner = (acc = {}, v, k) => {
    acc[k] = v;
    return acc;
  },
...
export const transduceBF = ({
  ...
  let queue = [];
  const pushNextQueueItems = childrenLoopReducer((aa, vv, kk, cc) => {
    // push next level
  ...
  const reduceItem = compose(
    ...
    (nextReducer) => (a, v, k, c) => {
      ...
      if (queue.length > 0) reduceItem(...queue.shift());
    },
    visit,
    postVisit
  )(edgeCombiner);
...
export const memoize = (fn, by = identity) => {
  const mFn = (...x) => {
    const k = by(...x);
    return fn(
      ...(mFn.cache.has(k) ? mFn.cache.get(k) : mFn.cache.set(k, x) && x)
    );
  };
...
export const tdKeyBy = (by = (x) => x.id) => (next) => (o, v, k, c) =>
  next(o, v, by(v, k, c), c);
...
export const diffObjs = (a = {}, b = {}) => {
  ...
  for (k in a)
    k in b
      ? (aibc += aub[k] = aib[k] =
          a[k] === b[k] ? 1 : (changedc += changed[k] = 1))
      : (anbc += aub[k] = anb[k] = 1);
  for (k in b)
    k in a
      ? k in aib
...
export const diffBy = (by = (x) => x.id, args = []) =>
  by ? diffObjs(...args.map(keyBy(by))) : diffObjs(args);

export const sortedRangeIsSubsetStrict = (sortedSubrange, sortedRange) =>
  sortedRange[0] <= sortedSubrange[0] && sortedRange[1] >= sortedSubrange[1];
export const sortedRangeIsSubsetOrEqual = (sortedSubrange, sortedRange) =>
  sortedRange[0] <= sortedSubrange[0] && sortedRange[1] >= sortedSubrange[1];
...
function symmetricDifference(setA, setB) {
  ...
  for (const elem of setB)
    _difference.has(elem) ? _difference.delete(elem) : _difference.add(elem);
  return _difference;
}
...
export const setNonEnumProp = (obj, key, value) =>
...
export const setImmutableNonEnumProp = (obj, key, value) =>
  Object.defineProperty(obj, key, {
    value,
    enumerable: false,
    writable: true,
...
export const assignEnumAndNonEnumProps = (dest, ...srcs) =>
  srcs.reduceRight((acc, src) => {
...
const combinations = (n, k) => fact(n) / (fact(k) * fact(n - k));
const permutations = (n, k) => fact(n) / fact(n - k);
const permutations_w_repetition = (n, k) => n ** k;
const combinations_w_repetition = (n, k) =>
...
if (process.env.NODE_ENV !== "production") {
  ...
  compose = (...fns) => {
    ...
    const composed = (...args) =>
      fns.reduceRight((acc, f) => f(acc), fn(...args));
  ...
  compose = (...fns) => (...args) => {
    ...
    return fns
      ...
      .reduceRight((acc, f) => f(acc), fns[fns.length - 1](...args));
  };
  // eslint-disable-next-line
