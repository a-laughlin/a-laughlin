# based on https://github.com/aj-cloete/pySSA
import numpy as np
import pandas as pd
...
yearly_ssa = mySSA(daily_volume)
# weekly_ssa = mySSA(daily_volume) #removing weekly since results are similar
yearly_ssa.embed(embedding_dimension=400, suspected_frequency=daily_volume['2018'].count(), verbose=False)
yearly_ssa.decompose(verbose=False);

hankel_matrices_yearly = [yearly_ssa.view_reconstruction(yearly_ssa.Xs[i], plot=False,return_df=True) for i in range(6)]

for ax,r,c,i in enum_axes(r=len(hankel_matrices_yearly)+1,c=1,w=16,h=2):
  if i==0:
    ax.set_title("Daily Volume")
    daily_volume.plot(ax=ax);
  else:
#     ax.set_title(f"Trend {i}");
    hankel_matrices_yearly[r-1].plot(ax=ax,lw=0.5)
  ax.set_xticks([])
  ax.set_yticks([])
  ax.set_xlabel(None)
"""
...
class mySSA(object):
    ...
    @staticmethod
    def get_contributions(X=None, s=None, plot=False):
    ...
    @classmethod
    def view_reconstruction(cls, *hankel, names=None, return_df=True, plot=False, symmetric_plots=False):
    ...
    def forecast_recurrent(self, steps_ahead=12, singular_values=None, plot=False, return_df=True, **plotargs):
